...
import type {
  OfficeNavigationItem,
  OfficeWorkbenchDefinition,
} from '../../models/ui/office-platform/office-navigation';
...
const personal = {
  ...
  items: [
    ...
    item('My work queue', 'lucideListTodo', 'work-queue', undefined, [
      C.Workflow.View,
      C.Workflow.Forward,
    ]),
...
const definitions: OfficeWorkbenchDefinition[] = [
  {
    ...
    summary:
      'Official intake, registration, routing, custody, retention and disposition of records.',
    ...
    navigation: [
      personal,
      {
        ...
        items: [
          item('Document intake', 'lucideScanLine', 'intake', undefined, [
            C.Record.Register,
            C.Record.Archive,
          ]),
          item('Incoming register', 'lucideInbox', 'incoming-register', undefined, [
            C.Record.View,
            C.Record.Archive,
          ]),
          item('Outgoing register', 'lucideSend', 'outgoing-register', undefined, [
            C.Record.View,
            C.Record.Archive,
          ]),
          item('Classification & indexing', 'lucideTags', 'classification', C.Record.Classify),
          item('Routing & distribution', 'lucideRoute', 'routing', undefined, [
            C.Record.Route,
            C.Workflow.Forward,
          ]),
          item('Records repository', 'lucideLibrary', 'repository', undefined, [
            C.Record.View,
            C.Record.Archive,
          ]),
          item('Archives', 'lucideArchive', 'archives', C.Record.Archive),
          item('Retention & disposition', 'lucideCalendarClock', 'retention', undefined, [
            C.Record.Archive,
            C.Record.Dispose,
          ]),
          item('Registry audit trail', 'lucideHistory', 'audit-trail', undefined, [
            C.Audit.View,
            C.Record.Archive,
          ]),
        ],
  ...
  },
  {
    ...
    label: 'Secretariat',
    summary:
      'Executive correspondence, signatures, meetings, minutes and administrative coordination.',
    landingRoute: 'overview',
    accessCapabilities: [C.Document.View, C.Workflow.Forward],
    navigation: [
      personal,
      {
        label: 'Secretariat operations',
        items: [
          ...
          item('Signature queue', 'lucidePenTool', 'signature-queue', undefined, [
            C.Document.Sign,
            C.Workflow.Forward,
          ]),
  ...
  },
  {
    ...
    label: 'Processing',
    summary:
      'Operational queues for assigned documents, reviews, recommendations and completed work.',
    landingRoute: 'overview',
    accessCapabilities: [C.Document.View, C.Workflow.Forward],
    navigation: [
      personal,
      {
        ...
        items: [
          ...
          item('Escalated items', 'lucideTriangleAlert', 'escalated', undefined, [
            C.Workflow.View,
            C.Workflow.Escalate,
          ]),
        ],
  ...
  },
  {
    ...
    navigation: [
      personal,
      {
        label: 'Authority desk',
        items: [
          item('Approval inbox', 'lucideBadgeCheck', 'approvals', undefined, [
            C.Document.Approve,
            C.Document.Sign,
          ]),
          item('Signature queue', 'lucidePenTool', 'signature-queue', C.Document.Sign),
          item('Escalations', 'lucideTriangleAlert', 'escalations', undefined, [
            C.Workflow.Escalate,
            C.Directive.Issue,
          ]),
          ...
          item('Decision audit trail', 'lucideHistory', 'audit-trail', undefined, [
            C.Audit.View,
            C.Directive.View,
          ]),
        ],
  ...
  },
  {
    ...
    summary:
      'Organisation structure, access control, policies, integrations and platform configuration.',
    landingRoute: 'overview',
  ...
  },
  {
    ...
    summary:
      'Independent, primarily read-only oversight of documents, access and lifecycle controls.',
    landingRoute: 'overview',
    ...
    navigation: [
      personal,
      {
        ...
        items: [
          ...
          item('Governance exceptions', 'lucideTimerReset', 'exceptions', C.Audit.View),
          item('Violations & investigations', 'lucideShieldAlert', 'violations', C.Audit.View),
          item('Periodic access reviews', 'lucideClipboardCheck', 'access-reviews', C.Audit.View),
          item('Legal & audit holds', 'lucideLockKeyhole', 'holds', C.Audit.View),
          item('Policy traceability', 'lucideGitBranch', 'traceability', C.Audit.View),
